...
function ReceivedInstanceFilter(dicom)
    ...
    if (mod ~= 'SR') and (mod ~= 'CT') and (mod ~= 'MG') and (mod ~= 'CR') and (mod ~= 'DX') and (mod ~= 'PT') and (mod ~= 'NM') then
        return false
    else
        return true
    end
...
function OnStoredInstance(instanceId)
	...
	-- local url = "http://guest:guest@rabbitmq:15672/api/exchanges/%2f/test/publish"
	-- local payload = '[[\"' .. temp_file_path .. '\"], {}, {\"chord\": null, \"callbacks\": null, \"errbacks\": null, \"chain\": null}]'

	-- local headers = {["content_type"] = "application/json"}
	
	-- local body = {["properties"]={"content_type"="application/json","headers"={"id"="f9c1b9e5-d5fb-42d5-9854-93311ba8449c","task"="celery_in.process"}},"routing_key"="test","payload" = payload ,"payload_encoding"="string"}
